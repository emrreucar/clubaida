...
import img1 from "/public/restaurant.jpg";
...
const RestaurantPageComp = () => {
  const { t } = useTranslation();

  const linkWay = [
    { href: "/", text: `${t("institutional")}` },
    { href: "/", text: `${t("services")}` },
    { href: "/services/restaurant", text: `${t("restaurant")}` },
  ];

  return (
    <div>
      <Banner img={img1} title={`${"restaurant"}`} linkWay={linkWay} />

      <div className="lg:flex lg:mt-12 lg:px-10 ">
        <div className="p-8 lg:flex-1 lg:flex lg:flex-col lg:items-start lg:justify-center ">
          <h1 className="text-2xl font-semibold"> {t("restaurant")} </h1>
          <p className="text-gray-400 mt-2">{t("restaurant_desc")}</p>
        </div>
...
export default RestaurantPageComp;
